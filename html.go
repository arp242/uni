...
/*
func main() {
	j, _ := ioutil.ReadFile("entities.json")
	...
	for ent, cp := range out {
		...
		if len(cp.Codepoints) != 1 {
			continue
		}

		found := false
		for _, s := range seen {
			if cp.Codepoints[0] == s {
				found = true
				break
			}
		}
		...
		fmt.Printf("\t%d: \"%s\",\n", cp.Codepoints[0], strings.Trim(ent, "&;"))
		seen = append(seen, cp.Codepoints[0])
	}
}
*/
