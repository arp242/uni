...
func mkemojidata() (closeErr error) {
	...
	for _, line := range strings.Split(string(text), "\n") {
		...
		// TODO: record this? I think just "skintone: true" might be enough?
		if strings.Contains(name, "light skin tone") ||
			strings.Contains(name, "medium-light skin tone") ||
			strings.Contains(name, "medium skin tone") ||
			strings.Contains(name, "medium-dark skin tone") ||
			strings.Contains(name, "dark skin tone") {
			continue
		}
		...
		_, err = fp.WriteString(fmt.Sprintf("\t{[]uint32{%s}, `%s`, `%s`, `%s`},\n",
			strings.Join(cp, ", "), name, group, subgroup))
		if err != nil {
			return err
		}
	}
